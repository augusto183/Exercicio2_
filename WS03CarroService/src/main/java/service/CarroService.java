...
public class CarroService {
	...
	public void makeForm(int tipo, Carro carro, int orderBy) {
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			String action = "/carro/";
			String name, marca, modelo, buttonLabel;
			if (tipo == FORM_INSERT){
				action += "insert";
				name = "Inserir Carro";
				marca = "Toyota, Ford, ...";
				modelo = "Corolla, Fiesta, ...";
				buttonLabel = "Inserir";
			} else {
			...
			umCarro += "\t\t\t<td>Preço: R$ "+ carro.getPreco() + "</td>";
			umCarro += "\t\t\t<td>&nbsp;</td>";
			umCarro += "\t\t</tr>";
			umCarro += "\t</table>";		
		} else {
		...
		for (Carro c : carros) {
			...
			list += "\n<tr bgcolor=\""+ bgcolor +"\">\n" + 
            		  ...
            		  "\t<td align=\"center\" valign=\"middle\"><a href=\"javascript:confirmarDeleteCarro('" + c.getID() + "', '" + c.getMarca() + " " + c.getModelo() + "', '" + c.getPreco() + "');\"><img src=\"/image/delete.png\" width=\"20\" height=\"20\"/></a></td>\n" +
            		  "</tr>\n";
		}
		list += "</table>";		
	...
	public Object insert(Request request, Response response) {
		...
		Carro carro = new Carro(-1, marca, modelo, ano, cor, preco);
		
		if(carroDAO.insert(carro) == true) {
            resp = "Carro (" + marca + " " + modelo + ") inserido!";
            response.status(201); // 201 Created
		} else {
			resp = "Carro (" + marca + " " + modelo + ") não inserido!";
